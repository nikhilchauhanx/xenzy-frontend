...
export default function SellerDashboardPage() {
  // Get the user and supabase client from our central context
  ...
  // This effect runs when the component loads or the user changes
  useEffect(() => {
    ...
    if (isAuthenticated && user) {
      const fetchMyProducts = async () => {
        ...
        setError('');
        try {
          // Fetch products directly from the 'products' table where the
          // 'user_id' column matches the currently logged-in user's ID.
  ...
  if (!isLoading && !isAuthenticated) {
    return (
      <div className="container mx-auto text-center py-20">
        ...
        <button onClick={() => router.push('/auth')} className="mt-4 bg-blue-500 text-white px-6 py-2 rounded-lg">
          Log In
        </button>
      </div>
    );
      ...
      <div className="lg:grid lg:grid-cols-4 lg:gap-12">
        ...
        <aside className="lg:col-span-1">
          <nav className="flex flex-col space-y-2">
            <Link href="/sell" className="bg-gray-100 text-gray-800 font-semibold px-4 py-3 rounded-lg flex items-center">
        ...
        <div className="lg:col-span-3 mt-10 lg:mt-0">
          <div className="flex justify-between items-center mb-6">
            ...
            <Link href="/sell/new" className="bg-blue-600 text-white px-5 py-2 rounded-lg font-semibold hover:bg-blue-700">
              Add New Listing
            </Link>
          </div>
                    ...
                    {myListings.length > 0 ? (
                      myListings.map((item) => (
                        <tr key={item.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-4">
                            {/* --- THIS IS THE FIX --- */}
                            {/* The href now points to the dynamic edit page for each product */}
                            <Link href={`/sell/edit/${item.id}`} className="text-blue-600 font-semibold hover:underline">
                              Edit
                            </Link>
                          </td>
                        </tr>
